...
#sig <-(1:N)*exp(4*(1:N)/(5000*N))*sin(2*pi*(1:N)/30)
plot(sig,type='l')
X<-hankel(sig,L=120)
...
for(k in 1:M)
{
  ...
  start.time<-Sys.time()
  s <- ssa(ser, L = 120)
  rec <- reconstruct(s, groups = list(c(1),c(2,3))) 
  end.time<-Sys.time()
  ...
  trend.season <- rec$F1+rec$F2

  ############################# #pcaL1(120.3)###############################
  X<-hankel(ser,L=120)
  start.time<-Sys.time()
  s120.L1svd<-l1pca(X,center=FALSE,projections="l1",projDim=3)
  end.time<-Sys.time()
  ...
  # Pr<-d.decr[1]*u.decr[,1]%*%t(v.decr[,1])+d.decr[2]*u.decr[,2]%*%t(v.decr[,2])+ d.decr[3]*u.decr[,3]%*%t(v.decr[,3])
  # end.time<-Sys.time()
